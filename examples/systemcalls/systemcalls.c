...
/*
** @param ret is the return value to be checked
** @return true if ret is -1, which normally indicates a fail
*/
bool is_error(int ret) { return ret == -1; }
 ...
 * exec() does not perform path expansion, the command to execute needs to be
 * an absolute path.
...
bool do_exec(int count, ...) {
  ...
  if (is_error(waitpid(pid, &status, 0))) {
    return false;
  }
